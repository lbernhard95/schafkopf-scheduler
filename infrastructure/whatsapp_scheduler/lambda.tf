...
resource "aws_iam_role" "whatsapp_scheduler_scheduler" {
  name = "whatsapp_scheduler_scheduler"
  assume_role_policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
...
resource "aws_iam_role_policy" "whatsapp_scheduler_scheduler" {
  role = aws_iam_role.whatsapp_scheduler_scheduler.id
  policy = jsonencode({
    Version = "2012-10-17"
    Statement = [
      {
        ...
        Resource = aws_lambda_function.whatsapp_scheduler.arn
      }
    ]
  })
}
...
resource "aws_lambda_permission" "allow_scheduler" {
  ...
  source_arn = aws_scheduler_schedule.whatsapp_scheduler.arn
}
